      ...
      .then(() => {
        return Word.run(async (context) => {
          try {
            ...
            // Setup WebSocket connection after getting email
            setupWebSocket(userEmail);

            // Open login page in background
...
function setupWebSocket(email) {
  ...
  ws = new WebSocket(`ws://localhost:3001?email=${email}`);
  ...
  ws.onmessage = async (event) => {
    ...
    console.log("Received passkey from WebSocket:", data);
    
    if (data.email === userEmail && data.passkey) {
      passkey = data.passkey;
      // Automatically verify the passkey
      await verifyPasskey(data.passkey, data.email);
    }
  };
...
// Modify verifyPage to use the automatically received passkey
async function verifyPasskey(receivedPasskey, receivedEmail) {
  ...
  try {
    ...
    message.innerText = "Error verifying passkey";
    message.style.color = "red";
  }
}
